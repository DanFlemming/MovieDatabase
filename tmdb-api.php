 ...
 * 		Image				http://cf2.imgobject.com/t/p/original/IMAGEN.jpg  
...
/*
		...
		$j = count($result['results']);
		
		for($i=0; $i<$j; $i++) {
			$movieId		= $result['results'][$i]['id'];
	
			$x = $this->getMovie($movieId);

  			$movieTitle 	= $x[$i]['title'];
			$moviePoster 	= $x[$i]['poster_path'];
			$movieOverview 	= $x[$i]['overview'];
			$movieRating 	= $x[$i]['vote_average'];
			$movieDate 		= $x[$i]['release_date'];
			$movieBackdrop 	= $x[$i]['backdrop_path'];

			$movieTrailer 	= $x[$i]['trailer'];
			
			add_movie($movieId,$movieTitle,$moviePoster,$movieOverview,$movieRating,$movieDate, $movieTrailer, $movieBackdrop );
			
		
			
			$movie = get_movie($movieId);
			$items[$i]['id']			= $movie['movie_id'];
			$items[$i]['title']			= $movie['movie_title'];
			$items[$i]['poster_path']	= $movie['movie_poster'];
			$items[$i]['overview']		= $movie['movie_overview'];
			$items[$i]['rating']		= $movie['movie_rating'];
			$items[$i]['release_date']	= $movie['movie_date'];
			$items[$i]['backdrop_path'] = $movie['movie_backdrop'];
			$items[$i]['trailer']		= $movie['movie_trailer'];
	...
	public function getMovie($idMovie, $appendToResponse = null) {
		...
		$result = $this->_call('movie/'.$idMovie, $appendToResponse, '&query='. urlencode($idMovie));
		//var_dump($result);
		...
		for($i=0; $i<$j; $i++) {
			...
			$items[$i]['poster_path']	= $result['poster_path'];
			$items[$i]['overview'] 		= $result['overview'];
			$items[$i]['vote_average'] 	= $result['vote_average'];
			$items[$i]['release_date'] 	= $result['release_date'];
			$items[$i]['backdrop_path'] = $result['backdrop_path'];
			
			//we need to set the array to null if it does not exiist or else 
			//we get a fatal error
			if(!isset($result['trailers']['youtube'][0]['source'])) {
				if(!isset($result['trailers']['youtube'][1]['source'])) {
					if(!isset($result['trailers']['youtube'][2]['source'])) {
						$items[$i]['trailer']		= "null";
					}	
				}
			}else
				$items[$i]['trailer']	= $result['trailers']['youtube'][0]['source'];
		}
		//var_dump($result['trailers']);
		return $items;
	}
	...
	public function searchMovie($movieTitle){
		...
		/*	$movieTitle 	= $result['results'][$i]['title'];
			$moviePoster 	= $result['results'][$i]['poster_path'];
			$movieOverview 	= $result['results'][$i]['overview'];
			$movieRating 	= $result['results'][$i]['vote_average'];
			$movieDate 		= $result['results'][$i]['release_date'];*/
			//$movieTrailer 	= $result['youtube'][$i]['source'];
			//$movies[] = new Movie($data);
			...
			$x = $this->getMovie($movieId);
  			...
  			//$yes = $x['id'][$i];
			//var_dump($x[0]['trailers']);
			
  		 
  			//take what it returns and set it to variable
  			$movieTitle 	= $x[$i]['title'];
			$moviePoster 	= $x[$i]['poster_path'];
			$movieOverview 	= $x[$i]['overview'];
			$movieRating 	= $x[$i]['vote_average'];
			$movieDate 		= $x[$i]['release_date'];
			$movieBackdrop 	= $x[$i]['backdrop_path'];
			//$movieTrailers[$i]['trailer'] 	= $x[$i]['trailers']['youtube'] ;
			$movieTrailer 	= $x[$i]['trailer'];
			//now pass all of theres into the database !! and were accessing a lot more
			//data
			...
			//$test[$i]['id'] = get_movie($movieId);
			//$test[$i]['title'] = get_movie($movieId) . "<br>";
			...
			$items[$i]['backdrop_path'] = $movie['movie_backdrop'];
			$items[$i]['trailer']		= $movie['movie_trailer'];
			//echo $movieTrailer. "<br>";
			//var_dump($movieTrailer);

			//echo $items['id'];
			//echo $items[$movieId]['title'] . "<br>";
		...
		//echo $test[0]['title'];
		//echo $test[0]['id'];
		//echo $test[0]['poster_path'];
		///echo $test[1]['title'];
		//echo count($test);
	/*var_dump($movieTrailers[0]['source']);
	echo $movietrailer;*/
	//var_dump($x);
		return $items;
	}
